...
const OfferDetails = () => {
  ...
  for (const city in offerHotels) {
    const foundHotel = offerHotels[city].find((h) => h.id === id);
    if (foundHotel) {
      hotel = foundHotel;
      cityName = city;
      break;
  ...
  const [roomsData, setRoomsData] = useState([{ adults: 2, children: 0 }]);
  const [showGuestsPicker, setShowGuestsPicker] = useState(false);
  const [hasGST, setHasGST] = useState(false);



  const handleRoomChange = (idx, type, delta) => {
    setRoomsData((prev) => {
      const updated = [...prev];
      updated[idx][type] = Math.max(0, updated[idx][type] + delta);
      return updated;
    });
    updateGuestsSummary();
  };
  ...
  const addRoom = () => {
    setRoomsData([...roomsData, { adults: 2, children: 0 }]);
    updateGuestsSummary();
  };
  ...
  const updateGuestsSummary = () => {
    ...
    setGuests(
      `${roomsData.length} Room${roomsData.length > 1 ? "s" : ""}, ${totalAdults} Adults${
...
const discountedPrice = selectedRoom ? selectedRoom.discountedPrice : 0;
const discountPercent = 10;
const discountAmount = (discountedPrice * discountPercent) / 100;
const totalRoomPrice = discountedPrice * nights * totalRooms;
const totalDiscount = discountAmount * nights * totalRooms;
const totalTaxes = selectedRoom ? selectedRoom.taxes * nights * totalRooms : 0;
const totalPrice = totalRoomPrice - totalDiscount + totalTaxes;
...
const handlePayNow = () => {
  ...
  if (!hotel) {
    return <p className="text-center text-xl mt-10 text-red-500">Hotel not found.</p>;
  }

  return (
    <div className="max-w-2xl mx-auto p-4">
      <h2 className="text-2xl font-semibold mb-4 text-center">{hotel.name}</h2>
      <Slider {...sliderSettings}>
        {hotel.images.map((img, i) => (
          <div key={i}>
            <img src={img} alt={`Slide ${i}`} className="w-full h-96 object-cover rounded-md" />
          </div>
        ))}
      </Slider>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div className="md:col-span-2">
          {/* Amenities */}
          <h2 className="text-xl font-semibold mb-2 mt-4">Amenities</h2>
            ...
            <div className="relative">
              ...
              {showGuestsPicker && (
                <div className="absolute z-20 bg-white border rounded p-4 shadow mt-1 w-full">
                    ...
                    <div key={idx} className="mb-4 border-b pb-3">
                      ...
                      <div className="mb-2">
                        ...
                        <div className="flex items-center gap-2">
                          <button onClick={() => handleRoomChange(idx, "adults", -1)} className="border px-2 py-1 rounded">
                            <Minus size={14} />
                          </button>
                      ...
                      <div>
                        ...
                        <div className="flex items-center gap-2">
                          <button onClick={() => handleRoomChange(idx, "children", -1)} className="border px-2 py-1 rounded">
                            <Minus size={14} />
                          </button>
                  ...
                  <button onClick={addRoom} className="bg-blue-600 text-white px-4 py-2 rounded w-full">
                    Add Room
                  </button>
                </div>
              )}
        ...
        <div className="md:col-span-1 space-y-8">
          ...
          <div>
            ...
            <div className="border p-4 rounded-md space-y-2 text-sm text-gray-700">
          ...
          <div className="p-4 border rounded-xl shadow-sm space-y-4 text-sm">
            <label className="flex items-center gap-2 text-sm font-medium">
              ...
              <div className="relative">
                <input
                  ...
                  className="peer appearance-none w-5 h-5 border border-red-500 rounded bg-white checked:bg-red-500"
                />
                <svg className="absolute inset-0 w-5 h-5 text-white pointer-events-none hidden peer-checked:block" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round">
            ...
            {hasGST && (
              <div className="space-y-3">
                <input type="text" placeholder="GSTIN" className="w-full border p-2 rounded-md text-sm" />
                <input type="text" placeholder="Business Name" className="w-full border p-2 rounded-md text-sm" />
                <input type="text" placeholder="Business Address" className="w-full border p-2 rounded-md text-sm" />
                <input type="email" placeholder="Business Email" className="w-full border p-2 rounded-md text-sm" />
          ...
          <div className="mt-10">
            ...
            <div className="flex items-center gap-12 mb-4">
              <div>
                ...
                <div className="border rounded-md px-4 py-2 text-center font-semibold text-sm shadow-sm w-28">12:00 PM</div>
              </div>
              <div>
                <p className="text-xs text-gray-500">Check-out</p>
                <div className="border rounded-md px-4 py-2 text-center font-semibold text-sm shadow-sm w-28">11:00 AM</div>
              </div>
            </div>
            <ul className="list-disc list-inside text-sm text-gray-700 space-y-1">
              ...
              <li>This hotel is independently managed and operated under a licensed hospitality brand.</li>
        ...
        <div>
          <div className="border p-6 rounded-xl shadow-sm sticky top-4 mt-4">
